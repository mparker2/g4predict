...
def filter_overlapping(cluster):
    ...
    for i, record in enumerate(end_sorted_cluster):
        ...
        # calculate the score from previous records if not including this record
        not_incl = max(max_of_cluster[i][:2])
        
        # find the closest non-overlapping record on the left of the current one
        closest = bisect_left(end_sorted_vals, record[1]) - 1
        
        # score including this record is cumscore from closest plus score of current
...
def merge_overlapping(cluster):
    ...
    some indication of G4 polymorphism at the locus, though the regex does
    not capture all polymorphisms).
    '''
